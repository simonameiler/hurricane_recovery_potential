...
recovery simulations for US Atlantic- and Gulf-coast counties.
...
|----------|---------------|
| `notebooks/probabilistic_analysis.ipynb` | Fig. 2 `annual_3panel.png`, Fig. 3 `recovery_drivers_annual_vs_median.png`, Fig. 4 `bivariate_map_B_risk_vs_capacity.png`; SI: `na_coast_hazard_overview.png`, `median_event_3panel.png`, `max_event_3panel.png`, `recovery_drivers_annual_max.png`, `skewness_wd.png`, Fig. S9 `scaling_valueadd.png` (multi-hazard scaling value-add), and the NRI evaluation (`table_nri_evaluation.tex`, `table_divergent_counties.tex`, `nri_*.csv`) |
| `notebooks/historical_analysis.ipynb` | Fig. 1 `hist_AL132020_4panel_focus.png` (Laura) and `table_laura_main.tex`; SI: `hist_AL142018_4panel_focus.png` (Michael), `table_laura_SI.tex`, `table_damage_distribution.tex`, `table_damage_normalized.tex`, `table_scaling_robustness.tex`, `table_huang_thresholds.tex`, and `table_scaling_robustness_all_storms_with_diff.csv` (all-storm wind-only-vs-scaled FEMA-correlation sensitivity, computed in-notebook) |
...
the five county-level metric CSVs in `analysis_output/`, and the Figure S9
multi-hazard scaling value-add tables (`analysis_output/scaling_valueadd_*.csv`,
`analysis_output/scaling_sensitivity/g0.0/{recovery_potential,earp_per_county}.csv`;
see "Figure S9 — multi-hazard scaling value-add" below).
...
### Figure S9 — multi-hazard scaling value-add

Quantifies the effect of the rain/surge scaling on repair demand and recovery
burden by comparing the wind-only (scaling_strength g=0) and manuscript
default (g=1) specifications. Both are exact: the per-event impact CSVs
already cache both damage-state counts, so no re-run of Stage 4 (CLIMADA
ImpactCalc) is needed.

```bash
python scripts/compute_scaling_valueadd.py
# -> analysis_output/scaling_valueadd_by_county.csv, scaling_valueadd_ds_totals.csv,
#    scaling_valueadd_transition_matrix_{absolute,normalized}.csv

# The recovery-burden (EARB) panel additionally needs a g=0.0 rerun of Stage 5,
# built from the cached wind-only per-event counts (no CLIMADA re-run):
python scripts/build_scaling_sensitivity_specs.py
# -> data/impact_sensitivity/g0.0/per_event/all_events.csv
python scripts/run_scaling_sensitivity_pipeline.py
# -> analysis_output/scaling_sensitivity/g0.0/{recovery_potential,earp_per_county}.csv
...
| `modules/scaling_sensitivity.py` | `compute_scaling_valueadd.py` (Fig. S9 transition matrix) |
