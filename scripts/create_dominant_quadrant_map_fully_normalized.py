...
Uses FULLY NORMALIZED quadrant assignments (both damage and capacity normalized).
"""
...
# Load quadrant data (FULLY NORMALIZED VERSION)
quadrant_file = OUTPUT_DIR / 'event_county_quadrants_fully_normalized.csv'
df = pd.read_csv(quadrant_file)

print(f"Loaded {len(df):,} event-county pairs from fully normalized analysis")
...
for fips in df['fips'].unique():
    ...
    # Calculate average normalized metrics for this county
    avg_pct_damaged = county_data['pct_housing_damaged'].mean()
    avg_capacity_norm = county_data['capacity_per_1000_units'].mean()
    avg_recovery_years = county_data['recovery_months'].mean() / 12
    ...
    county_quadrants.append({
        ...
        'avg_pct_housing_damaged': avg_pct_damaged,
        'avg_capacity_per_1000_units': avg_capacity_norm,
        'avg_recovery_years': avg_recovery_years
    })

county_df = pd.DataFrame(county_quadrants)

# Save summary
summary_file = OUTPUT_DIR / 'dominant_quadrant_by_county_fully_normalized.csv'
...
ax.set_aspect('equal')
ax.axis('off')
ax.set_title('Dominant Recovery Vulnerability Quadrant by County\n(Fully Normalized: Both Damage and Capacity per Housing Stock)', 
             fontsize=14, pad=20, fontweight='bold')
ax.legend(handles=legend_elements, loc='lower left', fontsize=11, framealpha=0.95)

# Add subtitle with explanation
fig.text(0.5, 0.02, 
         'Damage: % of county housing damaged | Capacity: permits per 1000 housing units per month\n' +
         'Quadrant assignment based on most frequent classification across all hurricane events',
         ha='center', fontsize=9, style='italic', color='gray')

# Save
output_file = OUTPUT_DIR / 'dominant_quadrant_map_fully_normalized.png'
...
for quadrant in colors.keys():
    ...
    avg_recovery = county_df[county_df['dominant_quadrant'] == quadrant]['avg_recovery_years'].mean()
    print(f"{quadrant}: {count} counties ({pct:.1f}%) - Avg recovery: {avg_recovery:.1f} years")

print("\n=== SUMMARY STATISTICS BY QUADRANT ===")
for quadrant in colors.keys():
    subset = county_df[county_df['dominant_quadrant'] == quadrant]
    if len(subset) > 0:
        print(f"\n{quadrant}:")
        print(f"  Avg % housing damaged: {subset['avg_pct_housing_damaged'].mean():.2f}%")
        print(f"  Avg capacity per 1000 units: {subset['avg_capacity_per_1000_units'].mean():.3f}")
        print(f"  Avg recovery time: {subset['avg_recovery_years'].mean():.1f} years")
