"""
Create 3-panel maps for the 5 highest weighted damage events.
...
# Load highest damage events summary
highest_damage_file = OUTPUT_DIR / 'highest_damage_events_summary_weightedunits.csv'
highest_damage = pd.read_csv(highest_damage_file)
event_ids = highest_damage['event'].tolist()

print(f"Creating maps for {len(event_ids)} highest damage events: {event_ids}")
...
for event_id in event_ids:
    ...
    # Overall title
    total_damage = highest_damage[highest_damage['event'] == event_id]['total_weighted_damage'].values[0]
    median_recovery = event_data['recovery_months'].median()
    critical_pct = highest_damage[highest_damage['event'] == event_id]['critical_pct'].values[0]
    
    plt.suptitle(f'Event {event_id}: Total Weighted Damage = {total_damage:,.0f}, Median Recovery = {median_recovery:.0f} months, Critical = {critical_pct:.1f}%',
                 fontsize=13, y=0.98)
    
    plt.tight_layout()
    
    # Save
    output_file = OUTPUT_DIR / f'highest_damage_event_{event_id}_weightedunits_3panel.png'
    plt.savefig(output_file, dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"Saved: {output_file}")

print("\n=== All highest damage event maps created ===")
